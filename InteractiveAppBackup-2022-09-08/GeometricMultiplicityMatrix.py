...
# This file was *autogenerated* from the file GeometricMultiplicityMatrix.sage
...
def resolve(orbit, rankTriags, restr_dict, dim_dict, pos):
    """
    ...
    restr_dict[orbit][orbit][dim_dict[orbit]] = _sage_const_1 
    ...
    if defect > _sage_const_0 :
        non_semi_small(orbit, restr_dict, dim_dict, fibers, defect, pos)
    elif defect == _sage_const_0 :
        semi_small(orbit, restr_dict, dim_dict, fibers, pos)
        
        
def non_semi_small(orbit, restr_dict, dim_dict, fibers, defect, pos):
    """
    Computes the IC stalk shifts for the singular orbit inputted using
    ...
    multiplicities = dict([(C,{}) for C in fibers.keys()])
    ...
    while computed != set(fibers.keys()):
        ...
        for C in iter_list:
            ...
            if interval.issubset(computed):
                ...
                multiplicities[C] = {-i:fibers[C].get(dim_dict[C]-i,_sage_const_0 ) for i in range(_sage_const_0 ,int(defect)+_sage_const_1 )} 
                # Next we can obtain the other multiplicities by symmetry using Poincare-Verdier duality
                for i in range(_sage_const_1 ,int(defect)+_sage_const_1 ):
                        multiplicities[C][i] = multiplicities[C][-i]
                ...
                for key in restr_dict[orbit][C].keys():
                    restr_dict[orbit][C][key] = restr_dict[orbit][C][key] - sum([multiplicities[C2][shift]*
                                                                                 restr_dict[C2][C].get(key-shift,_sage_const_0 ) 
                                                                                 for C2 in interval for shift 
                                                                                 in range(-_sage_const_1 *int(defect),int(defect)+_sage_const_1 )])
        ...
        restr_dict[orbit][C] = {key:restr_dict[orbit][C][key] for key in restr_dict[orbit][C].keys() if restr_dict[orbit][C][key] > _sage_const_0 }
        
        
        
def semi_small(orbit, restr_dict, dim_dict, fibers, pos):
    """
    Computes the IC stalk shifts for the singular orbit inputted using
    ...
    multiplicities = dict([(C,_sage_const_0 ) for C in fibers.keys()])
    ...
    while computed != set(fibers.keys()):
        ...
        for C in iter_list:
            ...
            if interval.issubset(computed):
                ...
                if dim_dict[C] in fibers[C].keys():
                    multiplicities[C] = fibers[C][dim_dict[C]]
                    
                # Initialize the shift dictionary for the orbit at C to be C's fiber's cohomology
                restr_dict[orbit][C] = fibers[C]
                
                # Use the fact that IC(C')|C is contained in degrees D, dim C < D <= dim C' for C < C'
                # to set dim C shifts to 0
                restr_dict[orbit][C][dim_dict[C]] = _sage_const_0 
                
                # Subtract off the terms occuring in other IC's according to their multiplicities
                for C2 in interval:
                    restr_dict[orbit][C] = {key: restr_dict[orbit][C][key] - 
                                            multiplicities[C2]* restr_dict[C2][C].get(key,_sage_const_0 )
                                            for key in restr_dict[orbit][C].keys()}
                # Add C to the set of computed orbits
                computed.add(C)
            else:
...
def product_cohom(cohom, list_of_cohom):
    """
    ...
    if len(list_of_cohom) > _sage_const_1 :
        return product_cohom(cohom,[product_cohom(list_of_cohom[_sage_const_0 ],list_of_cohom[_sage_const_1 :])])
    elif len(list_of_cohom) == _sage_const_1 : # Else, use distributivity to compute the product explicitly
        # Initialize dictionary for storing the product
        cohom_dict = {}
        # For all pairs of shifts, increment the occurences of the sum of the shifts by their original product
        for i in cohom.keys():
            for j in list_of_cohom[_sage_const_0 ].keys():
                if not i+j in cohom_dict.keys():
                    cohom_dict[i+j] = cohom[i]*list_of_cohom[_sage_const_0 ][j]
                else:
                    cohom_dict[i+j] += cohom[i]*list_of_cohom[_sage_const_0 ][j]
        return cohom_dict
...
def grassman_cohom(grassman_pair, shift = _sage_const_0 ):
    """
    ...
    cohom_dict = dict([(-_sage_const_2 *i + shift,partition_dict[i]) for i in partition_dict.keys()])
...
def fiber(res_orbit, sub_orbit, dim_dict):
    """
    Computes the fiber of an element in sub_orbit in the resolution of res_orbit
        CURRENTLY NOT CORRECT FORMULAS, JUST FOR TESTING
    """
    ...
    if length == _sage_const_2 : 
        ...
        for i in range(len(res_diags[_sage_const_0 ])):
            fiber_list.append(grassman_cohom((res_diags[_sage_const_0 ][i]-sub_diags[_sage_const_0 ][i],
                                              diag[i]-sub_diags[_sage_const_0 ][i]), shift = dim_dict[res_orbit]))
        ...
        for i in range(len(res_diags[_sage_const_0 ])):
            if res_diags[-_sage_const_1 ][_sage_const_0 ] == _sage_const_0  and i == _sage_const_0 :
                fiber_list.append(grassman_cohom((res_diags[_sage_const_0 ][i]-sub_diags[_sage_const_0 ][i],
                                                  diag[i]-sub_diags[_sage_const_0 ][i]-sub_diags[_sage_const_0 ][i+_sage_const_1 ]), shift = dim_dict[res_orbit]))
    ...
    if len(fiber_list) == _sage_const_1 :
        return fiber_list[_sage_const_0 ], dimension
    elif len(fiber_list) > _sage_const_1 :
        return product_cohom(fiber_list[_sage_const_0 ],fiber_list[_sage_const_1 :]), dimension
    elif len(fiber_list) == _sage_const_0 :
        return {dim_dict[res_orbit]:_sage_const_1 }, dimension
...
def geomMatrix(infParam):
    ...
    restr_dict = dict([(C,dict([(C2,{}) for C2 in rankTriags])) for C in rankTriags])
    ...
    for C in smooth_orbs:
        for C2 in rankTriags:
            if C2 <= C:
                restr_dict[C][C2][dim_dict[C]] = _sage_const_1 
        ...
        for C in iter_list:
            if not C in computed:
                ...
                if set(pos.open_interval(C0,C)).issubset(computed):
                    ...
                    check = resolve(C, rankTriags, restr_dict, dim_dict, pos)
    ...
    return restr_dict, pos, rankTriags
    
    
    
def display_GeomMatrix(infParam, print_orbs = False):
    """
    ...
    restr_dict, pos, rankTriags = geomMatrix(infParam)
    ...
    for i in range(len(rankTriagsSorted)):
        ...
        for j in range(len(rankTriagsSorted)):
            shift_dict = IC_dict[rankTriagsSorted[j]]
            
            if len(shift_dict.keys()) > _sage_const_0 :
                for k in range(len(shift_dict.keys())):
                    shift = int(list(shift_dict.keys())[k])
                    multiplicity = int(shift_dict[shift])
                    if multiplicity > _sage_const_1 :
                        latex_str += str(multiplicity) + r"\mathbb{1}_{" + repr(rankTriagsSorted[j]) + r"}[" + str(shift) + r"]"
                    elif multiplicity == _sage_const_1 :
                        latex_str += r"\mathbb{1}_{" + repr(rankTriagsSorted[j]) + r"}[" + str(shift) + r"]"
                    else:
...
def display_SpecMatrix(infParam, print_multis = False):
    ...
    for i in range(len(rankTriagsSorted)):
        ...
        IC_dict = restr_dict[rankTriagsSorted[i]]
        for j in range(len(rankTriagsSorted)):
            shift_dict = restr_dict[rankTriagsSorted[j]][rankTriagsSorted[i]]
            
            if len(shift_dict.keys()) > _sage_const_0 :
                latex_str += " "+str(int(sum([shift_dict[a] for a in shift_dict.keys()])))+" "
            else:
                latex_str += " 0 "
...
def display_Standards(rankTriagsSorted):
    """
    Displays the standard representation associated to all the orbits in the rank triangle array
    """
...
def geom_matr(infParam, print_orbs = False):
    infParam = [float(i) for i in re.findall(r'[-+]?\d+\.?\d*', infParam)]
    
    display_GeomMatrix(infParam, print_orbs = print_orbs)
